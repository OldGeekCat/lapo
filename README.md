...
|---|---|---|---|
| action MSE（xyz） | _待补充_ | _待补充_ | _待补充_ |
| action MSE（rot6d） | _待补充_ | _待补充_ | _待补充_ |
| grip acc | _待补充_ | _待补充_ | _待补充_ |
| ep_mse（Stage 4 结束） | — | — | _待补充_ |
| endpoint_gain | — | _待补充_ | — |

复现：Stage 4 训练完成后运行 `scripts/lapo_eval.py`（oracle vs predictor）
与 `scripts/lapo_vs_xvla_eval.py`（baseline 对比），产出 `lapo_eval_report.json`。
