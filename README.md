...
即「先定终点，再推动作」（predict-then-infer）。endpoint 监督来自
**未来观测帧本身**（act-free，不依赖额外标注），动作监督仍是专家示教。
...
> 训练在远程 GPU 服务器进行（48h 全自动流程：
> `scripts/lapo_48h_full.sh` + orchestrator + monitor）。
> 最终数值以评估脚本产出的 `lapo_eval_report.json` 为准，填入下表：

| 口径 | X-VLA (flow matching baseline) | LAPo (oracle e) | LAPo (predictor e) |
|---|---|---|---|
| action MSE（xyz） | _TBD_ | _TBD_ | _TBD_ |
| action MSE（rot6d） | _TBD_ | _TBD_ | _TBD_ |
| grip acc | _TBD_ | _TBD_ | _TBD_ |
| ep_mse（Stage 4 结束） | — | — | _TBD_ |
| endpoint_gain | — | _TBD_ | — |
...
└── tests/                       # 105 passed（无 torch 环境可跑核心逻辑）
```
